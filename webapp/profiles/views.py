...
import json
class AddPostForm(TemplateView):
    template_name = 'profiles/add_post_form.html'
...
class AddNoteView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        note_rules = request.POST.get('note_rules')
        note_awards = request.POST.get('note_awards')
        ...
        if note_text and author and parent:
            note = parent.add_note(title = note_title, text=note_text, rules=note_rules, awards=note_awards, author=author, user=user, attachment=uploaded_file_url)
